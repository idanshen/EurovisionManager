...
def cmp_scores(item1, item2):
    ...
    score_diff = round(item1[1], 5) - round(item2[1], 5)
    if score_diff == 0.0:
...
def calc_contest(states, judges, aud_perc):
    ...
    for state in states:
        scores[state] = 0.0
    for state in states:
        add_votes(scores, states[state], aud_perc)
    for judgeId, judge in judges.items():
        if not judge.is_valid(states):
            continue
        for i, state in enumerate(judge.results):
            scores[state] += SCORES[i] * (100 - aud_perc)/100.0
